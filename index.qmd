...
```{r}
# Create manual labels for each variable
pretty_labels <- c(
  academic_workload   = "Academic Workload",
  academic_pressure   = "Academic Pressure",
  study_satisfaction  = "Study Satisfaction",
...
ggcorrplot(
  ...
  hc.order = TRUE,
  type = "lower",
  colors = c("#f45b5b", "white", "#0077b6"),  # low, mid, high
  outline.color = "white"
) +
...
gpa <- MentalHealthSurvey |> 
  ...
  geom_boxplot(aes(fill = cgpa), alpha = 0.5) + 
  theme_light() + 
  theme(
...
gpa_invest <- MentalHealthSurvey |> 
  ...
  facet_wrap2(~cgpa, strip = strip_themed(background_x = elem_list_rect(fill = c("#249EA080", "#00808380","#005F6080")))) + 
  theme(
    legend.position = "none",
...
total_mean <- Mean_mental_table |> 
  ...
  scale_fill_manual(
    values = c("Creative Outlets" = "#588B8B", "Online Entertainment" = "#FFFFFF", "Outdoor Activities" = "#FFD5C2", "Religious Activities" = "#F28F3B", "Sleep" = "#C8553D", "Social Connections" = "#2D3047","Sports & Fitness" = "#93B7Be")
  )
```
...
outlet_focus <- Mean_mental_table |> 
  ...
  facet_wrap2(~ Activity, 
              strip = strip_themed(
              background_x = elem_list_rect(fill = c("#588B8D80", "#FFD5C280", "#C8553D80"), alpha = 0.5), 
...
ggplot(MentalHealthSurvey, aes(x = sleep_relief, fill = average_sleep)) +
  ...
  scale_fill_manual(values = c("#EDAE4980", "#D1495B80", "#00798C80")) +
  theme_minimal() +
  labs(
...
MentalHealthSurvey %>% 
  ...
  ggplot(aes(x = sleep_cat, y = scores, fill = sleep_cat)) +
  geom_boxplot() +
...
[Download Presentation Slides](212%20Final%20Presentation.pptx)
...
[Download Presentation Video](https://raw.githubusercontent.com/mac-stat212-f25/project-model_citizens/main/212FinalPresentation%20(1)%20(1).mp4)
